...
?>
      ...
      <p><em><?php
printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'bwater' ),
   'href="' . get_permalink() . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'bwater' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark"',
...
if ( $categories_list && bwater_categorized_blog() ) :
?>
      <span class="cat-links">
         <?php printf( __( 'Posted in %1$s', 'bwater' ), $categories_list ); ?>
      </span>
      <?php endif; // End if categories ?>

<?php
   /* translators: used between list items, there is a space after the comma */
   $tags_list = get_the_tag_list( '', __( ', ', 'bwater' ) );
if ( $tags_list ) :
?>
      <span class="tag-links">
         <?php printf( __( 'Tagged %1$s', 'bwater' ), $tags_list ); ?>
      </span>
